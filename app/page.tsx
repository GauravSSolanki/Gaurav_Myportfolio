...
export default function Portfolio() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <Card className="bg-white border-gray-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
              ...
              <CardContent className="space-y-4">
                {education.map((edu, index) => (
                  <div
                    key={index}
                    className={`p-4 bg-gradient-to-r ${edu.gradient} rounded-lg border border-gray-200`}
                  >
                  ...
                  <div className="space-y-2">
                    {languages.map((lang, index) => (
                      <div key={index} className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 bg-gradient-to-r ${lang.color} rounded-full`}
                        ></div>
                        <p className="text-gray-700 font-medium">
      ...
      <section
        id="skills"
        className="py-20 px-4 bg-gradient-to-b from-white via-blue-50 to-white"
      >
        <div className="container mx-auto">
          ...
          <div className="max-w-5xl mx-auto mb-8">
            <div className="relative w-full h-64 mb-8 rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-r from-blue-600/80 to-purple-600/80 flex items-center justify-center">
              <div className="absolute inset-0 backdrop-blur-md bg-white/10"></div>
              <div className="relative z-10 text-center text-white">
                <Code className="w-16 h-16 mx-auto mb-4 drop-shadow-lg" />
                <h3 className="text-3xl font-bold drop-shadow">
                  MERN Stack Expertise
                </h3>
              </div>
            </div>
            <div className="flex flex-wrap justify-center gap-4 p-6 rounded-2xl bg-white/60 backdrop-blur-md shadow-lg">
              {skills.map((skill, index) => (
                <Badge
                  key={skill.name}
                  className={`
                    ${skill.color} text-white border-0 px-5 py-2 text-base font-semibold
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map((tech, techIndex) => (
                      <Badge
                        key={tech}
                        variant="secondary"
                        className="text-xs text-gray-700 border border-gray-200"
                      >
                        {tech}
                      </Badge>
                    ))}
            ...
            <div className="mt-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {organizations.map((org, index) => (
                  ...
                  >
                    <div
                      className={`h-1 bg-gradient-to-r ${org.gradient}`}
                    ></div>
                    <CardContent className="pt-6">
                      <div className="flex items-center gap-3 mb-2">
                        <div
                          className={`w-3 h-3 bg-gradient-to-r ${org.gradient} rounded-full`}
                        ></div>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {socialLinks.map((social, index) => {
              ...
              if (name === "linkedin") {
                ...
                bgColor = "bg-[#24292F] hover:bg-[#171a1d]";
              } else if (name === "email") {
                bgColor = "bg-gradient-to-r from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800";
              }
              return (
                <Button
                  key={social.name}
                  className={`${bgColor} text-white hover:brightness-110 transition-all duration-300 px-6 py-2 rounded-full font-semibold shadow-lg`}
                  asChild
                >
                  <a
                    href={social.url}
                  ...
                  >
                    ...
                    {name === "email" ? "Email Me" : social.name}
                  </a>
                </Button>
              );
            })}
